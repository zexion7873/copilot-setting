...
Flag any violation of these hard boundaries — full rules live in the stack modules under `instructions/` (the active skill's pre-load gate opens them):
...
- Never APPROVE without fresh `mvn` build/test evidence from the author — never infer a pass from the diff
