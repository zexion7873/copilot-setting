...
DENY_PATTERNS='rm -rf /([^a-zA-Z0-9]|$)|rm -rf \.( |$)|rm -rf \*|rm -rf \./\*|--no-preserve-root|sudo |doas |pkexec |DROP DATABASE|DROP SCHEMA|DROP TABLE|DROP INDEX|TRUNCATE|git push.*--force|git reset --hard|git clean -fd|chmod -R 777|mkfs\.|curl.*\|.*sh|wget.*\|.*sh|dd if=|kill -9 -1'
