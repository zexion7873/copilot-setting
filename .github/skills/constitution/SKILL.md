---
name: constitution
description: 'Use when user asks to define or update the project constitution — the non-negotiable principles and governance rules that supersede other practices. Triggers on: 寫 constitution, 定義原則, 訂專案原則, 更新治理規則, 修改 constitution, write constitution, define core principles, update governance, amend constitution, project principles. Produces or updates docs/constitution.md following Core Principles / Optional Sections / Governance / Version structure with semantic versioning (MAJOR for removal, MINOR for addition, PATCH for clarification) and a Sync Impact Report. Do NOT use for coding standards or file-type rules (prefer creating an instructions file), for architecture decisions on a single feature (prefer adr skill), for transient policies (constitution is for stable rules only), or for general documentation (Javadoc, API docs, migration guides are separate concerns).'
---
