...
This repo is **not application code** — it is a configuration distribution for **GitHub Copilot** built as two layers: a **generic process core** (the 9 skills, the `check-n-plus-1` / `find-impact` / `git-commit` prompts, hooks, and the validator — no stack names or versions anywhere, except the validator's canary anchor registry, its anchor-keyed test fixtures, and incidental examples in the hooks scripts and their tests) and a **swappable stack layer** (the agent `## Coding Standards` floor + persona lines plus residual stack mentions elsewhere in the agent files, the `instructions/` module set, the `check-tx` / `generate-migration-sql` prompts, the `copilot-instructions.md` Tech Stack declaration, and the canary anchor registry + anchor-keyed test fixtures inside the validator scripts). The shipped stack layer targets Java 8 / Maven / Spring Core 3.2 + Hibernate 4.2 (no Spring Boot — declarative transactions via XML `<tx:advice>`); porting to another stack swaps the stack layer only (`.github/STYLE-GUIDE.md` → File Lifecycle → Swapping the Stack Layer). Everything under `.github/` is content loaded by Copilot at runtime (agents, skills, instructions, prompts, hooks). When editing, you are editing prompt-engineering artifacts, not source code.

The target audience of the artifacts here is **Copilot users working in downstream repos** (Java ones, for the shipped stack layer), not this repo itself. There is no build, no test suite, and no runtime — only Markdown content and one validation script.
...
| Skills (9) | `.github/skills/<name>/SKILL.md` | Step-by-step workflow process (output templates embedded) | Description matches user intent, or `/skill-name` |
| Instructions (8) | `.github/instructions/*.instructions.md` | Stack modules — single source of truth for coding conventions (swappable stack layer) | A file matching `applyTo` glob is explicitly in context at request time (e.g. via `#file:`, editor attachment), or the model loads it on demand via semantic match on its `description` (discretionary); `applyTo: "**"` loads on every request |
...
Because most skill invocations happen without an attached file and on-demand loading cannot be relied on, the stack layer's hard-boundary rules (shipped stack: Java 8 / Spring 3.2 / Hibernate 4.2 / SQL / security, plus the `pom.xml` drift sentinel) are embedded directly in the code-touching agent bodies (`implementer`, `reviewer`, `debugger`) under `## Coding Standards`, loading deterministically when the agent is selected. Code-touching skills (`implement`, `refactor`, `code-review`, `sql-review`, `security-audit`, `debug`, `verify`) direct the model to open the stack modules under `instructions/` via their Phase 0 pre-load gate — by directory and module role, never by hardcoded filename, so the skills stay portable across stack layers. The files under `instructions/` remain the single source of truth; the agent-body embed is a deliberately minimal hard-boundary floor, not a full copy.
...
| `@implementer` | GPT-5.3-Codex | `implement`, `refactor` |
